...
the scorer sees which mark the ensemble preferred, not only the one it picked.
This is read straight from the bundled ONNX with onnxruntime (`arbtok/_ensemble.py`). The member weights are the TigreGotico rawi family (Apache-2.0, published on PyPI
...
## Why the lattice is a *second scorer*, not only a filter
...
- **The licensing filter (hard).** The candidate set is whatever the *variety's
  own* grapheme table admits. rawi is MSA-trained and has never seen the dialect. The spec has. This is a per-position constraint distinct from the generator's
...
- **The lattice cost (soft).** `latticeCost` is summed from the variety's
  *allophone-rescored* lattice, sun-letter assimilation, waṣl, emphatic backing,
  ...
  path *in that dialect* can outrank rawi's MSA-favored argmax.
...
---
[← Diacritization](tashkeel.md) · [Home](../README.md) · [Varieties →](dialects.md)
