...
## Why it is a separate path from loanword nativisation

A Latin run in Arabic text has two very different sources, and they need opposite
treatment:

* a **foreign loanword** — `meeting`, `manager`, `email` — is *nativised*: read
...
likely mater too, because Arabizi (unlike Arabic script) *spells its vowels* —
`habibi` is written, not `hbb`, so keeping the mater preserves the word shape for the
...
What v1 deliberately does **not** do: sniff all-alphabetic Arabizi (`habibi`, `inta`)
apart from English by morphology or lexicon. That call is real but error-prone, and a
wrong "yes" silently corrupts a genuine English embed — so v1 refuses it unless the
caller hints. This is the honest ambiguity: `habibi` with no hint stays on the
loanword path. Two conventions are also genuinely ambiguous and take a documented v1
...
p.transcribe("عِنْدِي meeting مَعَ manager") # digit-free English → nativised loans
...
leaves English embeds nativised, reverse-transliteration produces a plausible Arabic
...
`data/gold_code_switched/*.tsv` (the 99 rows once marked `unsupported`). Two
independent limits, measured honestly:
...
* **The diacritizer is the real ceiling.** Fed the *perfect* hand-spelt Arabic for
  each row, the current fusion diacritizer + lect phonology reproduces the
  hand-derived IPA at only **1.2 % exact / 47 % token** — because that gold was
  authored precisely *because* the pipeline does not produce it (dialectal
  monophthongisation, kashkasha, emphatic spread the diacritizer does not model).

End-to-end the Arabizi path reaches **0 % exact / 12.6 % token** on those 99 rows.
The rows therefore **stay `unsupported`**: overwriting citation-backed hand gold with
demonstrably-inferior pipeline output would launder a regression, not fix one. The
gate is diacritizer-bound, not input-path-bound — improving it is downstream work on
fusion and the dialect phonologies, tracked separately. The rows remain the standing
acceptance test that will flip to `pinned` when the diacritizer earns it.
