...
def test_the_french_rhotic_is_a_trill_not_a_velar_fricative(lect):
    """Kenstowicz & Louriz 2009; Ziadna 2018 p.185; Oueslati 2021 p.100. Each lect
    declares /ɣ/ and does not use it here, which is why the metric gets it wrong."""
...
def test_garage(lect, expected):
    ...
    [gɑːrˤɑːʤ]; the emphasis and the affrication are his own Setifian and are not
    encoded (see the table's comment on the Algerian split)."""
    assert transliterate("garage", lect, donor="fr-FR") == expected


def test_english_is_unaffected_on_these_lects():
    """The donor is still chosen by script, so an English code-switch is still read
    as English. The table's French-only segments never occur in en-GB output."""
    assert transliterate("weekend", "ar-MA") == transliterate("weekend", "ar-MA", donor="en-GB")
    for seg in ("ʁ", "y", "ø", "œ"):
        assert seg in translit._MAGHREBI_MAP
