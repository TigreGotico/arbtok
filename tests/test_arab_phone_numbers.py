"""A phone number of any Arab League member is read digit by digit under KSA_VOICE_AGENT,
...
from arbtok.textnorm import ARAB_PHONE_REGIONS, KSA_VOICE_AGENT, TtsNorm, normalize_for_tts

WORDS = dict(zip("0123456789٠١٢٣٤٥٦٧٨٩", ("صفر", "واحد", "اثنين", "ثلاثة", "أربعة", "خمسة",
                                          "ستة", "سبعة", "ثمانية", "تسعة") * 2))
NO_REGIONS = dataclasses.replace(KSA_VOICE_AGENT, phone_regions=())
...
def test_a_phone_number_is_read_digit_by_digit(region, written):
    said = normalize_for_tts(f"اتصل على {written} اليوم", "ar", KSA_VOICE_AGENT)
...
def test_a_number_without_a_trunk_prefix_is_read_after_a_word_that_names_it(region, written):
    assert normalize_for_tts(f"رقمي {written}", "ar", KSA_VOICE_AGENT) == f"رقمي {one_by_one(written)}"


def test_a_bare_kuwaiti_mobile_stays_a_number_and_after_raqmi_is_read_out():
    # libphonenumber example number, MOBILE, KW
    assert normalize_for_tts("50012345", "ar", KSA_VOICE_AGENT) \
        == normalize_for_tts("50012345", "ar", NO_REGIONS)
    assert "صفر" not in normalize_for_tts("50012345", "ar", KSA_VOICE_AGENT)
    assert normalize_for_tts("رقمي 50012345", "ar", KSA_VOICE_AGENT) == "رقمي " + one_by_one("50012345")


def test_arabic_indic_digits_are_read_as_ascii_ones_are():
    # libphonenumber example number, MOBILE, EG, in Arabic-Indic digits
    said = normalize_for_tts("اتصل على ٠١٠ ٠١٢٣٤٥٦٧ اليوم", "ar", KSA_VOICE_AGENT)
...
def test_a_number_in_the_other_digit_script_is_not_taken_into_a_phone_number():
    ...
    assert normalize_for_tts(written, "ar", KSA_VOICE_AGENT) == normalize_for_tts(written, "ar", NO_REGIONS)


def test_a_number_after_the_phone_number_is_not_taken_into_it():
    # libphonenumber example number, MOBILE, EG
    said = normalize_for_tts("اتصل على 010 01234567 3 مرات", "ar", KSA_VOICE_AGENT)
...
def test_a_date_a_time_a_price_or_a_count_is_not_a_phone_number(written):
    said = normalize_for_tts(written, "ar", KSA_VOICE_AGENT)
...
def test_a_saudi_service_number_or_landline_is_read_digit_by_digit(written):
    assert normalize_for_tts(f"اتصل على {written}", "ar", KSA_VOICE_AGENT) == f"اتصل على {one_by_one(written)}"
...
def test_reading_a_phone_number_loads_no_phone_number_library():
    code = ("import sys, arbtok\n"
            ...
            "print(normalize_for_tts('اتصل على +20 10 01234567', 'ar', KSA_VOICE_AGENT))\n"
