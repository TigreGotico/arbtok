"""A Saudi mobile number is read digit by digit under VOICE_AGENT, and a date that
...
def test_a_date_on_the_fifth_is_not_read_digit_by_digit(written):
    ...
    assert "صفر" not in normalize_for_tts(written, "ar", VOICE_AGENT)
...
def test_a_mobile_number_ends_at_its_tenth_digit():
    said = normalize_for_tts("0501234567 12 ريال", "ar", VOICE_AGENT)
