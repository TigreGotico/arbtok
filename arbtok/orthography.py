...
Provenance: vendored verbatim from ``text2tashkeel.orthography`` (TigreGotico/
text2tashkeel, Apache-2.0 — the same org and license as arbtok). These are the
decision-time constraints for the rawi class inventory arbtok bundles
(``arbtok/models/rawi_v2.vocab.json``); they are pure functions over that
inventory, so vendoring them alongside the model keeps arbtok self-contained.
